...
static std::optional<TransposeIndices> getIndicesFromInput(Value input) {
...
static std::optional<TransposeIndices> getIndicesFromOutput(Value output) {
  if (!isStaticallyShaped(output)) return std::nullopt;
  std::optional<linalg::TransposeOp> transposedOut;
...
struct ConvertConvNchwToNhwcPass
  ...
  void runOnOperation() override {
    ...
    // Propagate transposes up the graph.
    {
      ...
      (void)applyOpPatternsAndFold(reverseOps, frozenPatterns, config);
    }
